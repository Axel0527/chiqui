...
const MENU = [
  ...
  { id: 'asado',      name: 'Asado',      price: 40, img: 'azado.avif',      caja: 'aurora' },
...
function renderOrderFooter() {
  ...
  // Hint móvil
  const totalItems = Object.values(order).reduce((s, q) => s + q, 0);
  document.getElementById('hintAmount').textContent = fmt(total);
  document.getElementById('hintCount').textContent = totalItems + (totalItems === 1 ? ' item' : ' items');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const orderHeader = document.getElementById('orderPanelHeader');
  ...
  // Colapsar sheet al agregar desde menú en móvil (opcional: expandir automático)
  document.getElementById('menuAurora').addEventListener('click', () => {
    if (window.innerWidth <= 640) orderPanel.classList.add('expanded');
  });
  document.getElementById('menuXochil').addEventListener('click', () => {
    if (window.innerWidth <= 640) orderPanel.classList.add('expanded');
  });
